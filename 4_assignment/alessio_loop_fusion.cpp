...
struct MyLoopFusionPass: PassInfoMixin<MyLoopFusionPass> {
  ...
  bool areLoopsFuseable(Loop *LA, Loop *LB, DominatorTree &DT, PostDominatorTree &PDT, ScalarEvolution &SE, DependenceInfo &DI) {
    ...
    if (isLAGuarded != isLBGuarded) return false;
    ...
    if (!LA->isInnermost() || !LB->isInnermost()) return false;
    ...
    if (Guard1 && Guard1->getParent()->size() > 2) {
      errs() << "The second guard has other instructions besides the compare and the conditional branch.\n";
      return false;
    }
    ...
    // Note: The condition is valid only for positive steps.
    ...
    for (Instruction *I0 : MemInsts0) {
      for (Instruction *I1 : MemInsts1) {
        ...
        // The compiler doesn't know and assumes the worst case.
        ...
        // It is possible to obtain the SCEV by simulating that the instruction is in another loop.
        const SCEV *S0 = SE.getSCEVAtScope(Ptr0, L0); // It is equivalent to SE.getSCEV(Ptr0)
        const SCEV *S1 = SE.getSCEVAtScope(Ptr1, L0); // Instructions are moved from L1 to L0
        ...
        // Calculate distance: d(i) = S0(i) - S1(i).
        ...
        if (SE.isKnownNegative(Dist)) {
          ...
          // It includes cases where the two accesses have different steps.
          // Example:
          // S0 = Start0 + Step0 * i
          // S1 = Start1 + Step1 * i
          // If Step0 != Step1, then the distance depends on the induction variable i.
          errs() << "Distance cannot be statically proven non-negative.\n";
          return false;
        }
      }
    }
